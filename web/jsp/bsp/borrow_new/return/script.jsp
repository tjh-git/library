...
<script type="text/javascript">
    $("#code").focusout(function () {
       $.ajax({
           type:'get',
           url:'/borrow/return/returnBook',
           data:{code:$("#code").val()},
           success:function(data){
               var isOut=data.isOut;
               if(isOut==undefined)return;
               if(isOut==0)
               {
                   alert("该书籍未被借阅！");
                   return;
               ...
               if(data.fine!=0)
               {
                   $("#isOut").val("已逾期！罚款"+data.fine+"元")
               }
               $("#bookName").val(data.bookName);
               $("#borrowNum").val(data.borrowNum);
               $("#borrowTime").val(data.borrowTime);
               $("#borrowerCode").val(data.borrowerCode);
               $("#borrowerName").val(data.borrowerName);
               $("#fine").val(data.fine);
               $("#isOut").val(data.isOut);
               $("#money").val(data.money);
               $(".form-group").show(true);
           }
       })
    })
    $("#return_").click(function(){
        $.ajax({
            url:"/borrow/return/returnOneBook",
            data:{code:$("#code").val()},
            type:"post",
            success:function(data)
            {
