...
<script type="text/javascript">
  /*  $(document).ready(function(){
        $.ajax({
            type:'post',
            data:{key:'grade_code'},
            url:"/properties/getone",
            success:function(data)
            {
                var inner="";
                var one="<option value=\"#\">#</option>\n";
                for(var i=0;i<data.length;i++)
                {
                    inner+=(one.replace("#",data[i]).replace("#",data[i]));
                }
                console.log(inner);
                $("#grade").append(inner);
            }
    ...
    $("#submit").click(function(){
        var type=$("#id").val().toString();
        if(type.length==0)
        {
            $.ajax({
                type:'post',
                url:'/vip/student/addStudent',
                data:$('#addStudent').serializeArray(),
                success:function(data){
                    alert(data.mes);
                    $("#reset").trigger("click")
                }
        ...
        $.ajax({
            type:'post',
            url:'/vip/student/updateStudent',
            data:$('#addStudent').serializeArray(),
            success:function(data){
                alert(data.mes);
                $("#reset").trigger("click");
                window.location.href="/jsp/bsp/vip_new/studentList/blank.jsp";
            }
        })
    })
