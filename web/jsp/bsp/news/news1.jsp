...
<script type="text/javascript">
	...
	jQuery(function($){
		...
		$('#newsTable1').datagrid({
			...
			url:"${ctx}/news/queryNewsList1", 			//数据来源
			...
			toolbar:[
					{text:'预览', iconCls:'icon-edit',
						handler:function(){preview1();}
					}
					],
			//一定要加上这一句，要不然datagrid会记住之前的选择状态，删除时会出问题
			onLoadSuccess:function(){$('#newsTable1').datagrid('clearSelections');}
		});
	...
	//预览
		function preview1(){
			var rows = $('#newsTable1').datagrid('getSelections');
			if(rows.length==0){
	...
	function searchNews1(){
		var params = $('#newsTable1').datagrid('options').queryParams;
		//先取得 datagrid 的查询参数
		var fields =$('#searchNewsForm1').serializeArray(); //自动序列化表单元素为JSON对象
		$.each( fields, function(i, field){
			params[field.name] = field.value; //设置查询参数
		});
		$('#newsTable1').datagrid('reload'); //设置好查询参数 reload一下就可以了
	}
	//弹出窗口
	...
	//清空查询条件
	function clearNewsForm1(){
		$('#searchNewsForm1').form('clear');
	}
	
</script>

<form id="searchNewsForm1" name="searchNewsForm1">
	...
	<table style="width:98%; font-size:12px; border:0px solid #f0f0f0;">
		<tr>
			...
			<td width="13%" align="left">通知标题：</td>
			<td width="17%" align="left">
			...
			<td  width="38%" align="left">
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				<a href="#" onclick="searchNews1();" class="easyui-linkbutton" iconCls="icon-search">查询</a>
					&nbsp;&nbsp;&nbsp;	&nbsp;&nbsp;&nbsp;
				<a href="#" onclick="clearNewsForm1()" class="easyui-linkbutton" iconCls="icon-undo">清空</a>
			</td>  
			</tr>
	</table>
	</div>
