        ...
        success:function (data) {
            var grid_data=data.rows;*/
                ...
                jQuery(grid_selector).jqGrid({
                    mtype:'POST',
                    //direction: "rtl",
                    url:'/statistics/circulate/catalogList',
                    //subgrid options
                    subGrid : true,
                    sortname:'readCard',
                 ...
                 //   datatype: "local",
                    height: 250,
                    colNames:['类别', '图书种类','图书数量','图书金额','占比','查看记录'],
                    colModel:[
                        {name:'booktype',index:'booktype', width:100, sorttype:"int", editable: true,keys:true},
                        {name:'type_name',index:'type_name', width:250, sorttype:"int", editable: true,},
                        {name:'sum',index:'sum', width:80, sorttype:"int", editable: true},
                        {name:'sum_price',index:'sum_price',width:80, editable: true},
                        {name:'buy_rate',index:'buy_rate',width:50, editable: true},
                    ...
                    onClickButton: function () {
                        $.ajax({
                            ...
                            success:function(data){
                                ...
                                var str ="\ufeff";
                                str+=("类别, 图书种类,图书数量,图书金额,占比"+n);
                                data=data.rows;
                                var rows=data;
                                for(var i=0;i<rows.length;i++)
                                {
                                    var p="\ufeff"+rows[i].booktype+",";
                                    p+="\ufeff"+rows[i].type_name+",";
                                    p+="\ufeff"+rows[i].sum+",";
                                    p+="\ufeff"+rows[i].sum_price+",";
                                    p+="\ufeff"+rows[i].buy_rate+",";
...
<script>
    function putit(ele)
    {
        $("#type").val((ele.innerHTML));
        $("#show_kind").children().remove();
        $("#kind").hide();
    }
    $(document).ready(function(){
        $("#list-1").addClass("active open")
        $("#list-1-1").addClass("active");
        $("#type").on('input',function(){
            ...
            $.ajax({
                url:'/statistics/circulate/getAllType',
                type:'get',
                data:{show:dat},
                success:function(data)
                {
                    if(data.length==0)
                    {
                        var one="<p class=\"book_kind_info\" >没有匹配的类别！</p>";
                        $("#show_kind").children().remove();
                        $("#show_kind").append(one);
                        $("#kind").hide();
                        return;
                    }
                    $("#kind").show();
                    var all="";
                    var one="<p class=\"book_kind_info\" onClick=\"javascript:putit(this)\">&</p>";
                    for(var i=0;i<data.length;i++)
                    {
                        all+=(one.replace("&",data[i].TypeName));
                    }
                    $("#show_kind").children().remove();
                    $("#show_kind").append(all);
                }
            })
        })
    })
